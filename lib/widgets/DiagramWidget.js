...
export class DiagramWidget extends React.Component {
    ...
    if (action instanceof SelectingAction && multiselectDrag) {
      ...
      action.selectionModels.forEach(model => {
        if (model.model instanceof NodeModel || model.model instanceof PointModel) {
          ...
          // force nodes to snap to grid
          const gridSize = 30;
          model.model.x = Math.round(model.model.x / gridSize) * gridSize; 
          model.model.y = Math.round(model.model.y / gridSize) * gridSize;
        }
      });
  ...
  onMouseUp(event) {
    ...
    diagramEngine.clearRepaintEntities();
    if (actionOutput.type !== 'unknown' && actionOutput.type !== 'link-created') {
